...
deciParser :: Parser Double
deciParser =  do digits <- some digit
                 return $ read digits
...
floatParser = do r <- (literal "-") <||> deciParser
                 case r of
                   Left _ -> fmap (0-) deciParser
                   Right n -> return n

--parse spaces, throw them away
spaces :: Parser ()
spaces =  do rep (ws <|> singleLineComment <|> multiLineComment)
             return ()
ws =
  do sat (\c -> c == ' ')
     return ()

-- put comment code into space,
noReturn :: Char -> Bool
noReturn '\n' = False
noReturn x = isLatin1 x

--oneLineComment :: Parser Ast
singleLineComment =
  do literal "--"
     rep (sat noReturn)
     return ()

multiLineS =  (do _ <- literal "-}"; return () ) <|>
  (do (sat isLatin1)
      _ <- multiLineS
      return ())

--multLineComment :: Parser Ast
multiLineComment =
  do literal "{-"
     multiLineS
